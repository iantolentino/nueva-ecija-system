...
export default async function handler(req, res) {
  ...
  const households = await sql`
    ...
    LIMIT 100
  `;
  const rows = households.map((household) => `<tr>
    <td>${escapeHtml(household.address_line || 'Address not recorded')}</td>
    <td>${escapeHtml(household.barangay_name)}</td>
    <td>${household.member_count}</td>
    <td>${escapeHtml(household.members || 'No linked members')}</td>
  </tr>`).join('');
