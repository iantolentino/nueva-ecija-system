...
export function renderPublicLayout({ title, content, activePath = '/', alert }) {
  ...
  const links = [
    ...
    { href: '/public/hearings', label: 'Public Hearings' },
    ...
    { href: '/login', label: 'Staff Portal', className: 'btn btn-small' },
  ];
...
<body>
  <div class="public-shell">
    <header class="public-nav">
      <a class="public-brand" href="/">Nueva Ecija<br><span>Citizen Services Portal</span></a>
      <nav aria-label="Public services">
        ${links.map((link) => {
          const linkPath = link.href.replace(/\/+$/, '') || '/';
          const active = path === linkPath;
          return `<a class="${escapeHtml(link.className || 'public-nav-link')} ${active ? 'active' : ''}" href="${link.href}">${escapeHtml(link.label)}</a>`;
        }).join('')}
      </nav>
    </header>
    <main>
